...
export const specificOrder = async(req,res)=>{
  try {
    ...
    const key = `order:customer:${req.params.slug}`
    const order = await cacheResponse(key, 30, async () => {
      return await Order.find({ "shippingAddress.fullName":req.params.slug }).populate('user','name email').populate('items.product','name price')
    })
...
export const myOrder = async (req,res)=>{
  try {
    const key = `order:my:${req.user._id}`
    const order = await cacheResponse(key, 30, async () => {
      return await Order.find({user:req.user._id}).populate('items.product','name price').sort({createdAt:-1})
    })
    if(!order || order.length === 0) return res.status(404).json({message:"order not found"})
...
export const allOrders = async (req,res)=>{
  try {
    ...
    const key = `orders:all:${query}`
    const order = await cacheResponse(key, 30, async () => {
      return await Order.find(filter).populate('user','name email').populate('items.product','name price').sort({createdAt:-1})
    })
